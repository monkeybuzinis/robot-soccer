...
| 11 | Kalman velocity decay blocked during theta-rejected cycles | `stages-code/claude-laste.txt`: theta alternated 35.4°/−54.6° (bug #19 pose jump). During the −54.6° cycles, `ball_valid=true` (frozen readings below the 15-cycle BALL_FROZEN threshold), so `haveBallMeas=true` — but the 60° theta-jump check rejected every measurement and skipped `ballKF.update()`. The old decay condition `if (ballKF.inited && !haveBallMeas)` never fired because `haveBallMeas=true`, so KF velocity built from the pose jump propagated unchecked for 100–200+ consecutive cycles. `ball_now` drifted from ~500mm to 1652mm+; `kickPt` followed; `KICK_POINT_MISSED` fired before the robot could approach; `AT_KICK_POSE` never fired in the entire log. Same problem on `goalKF` — no velocity decay existed at all, so `goal_now` drifted to 2100mm+ despite the goal being a static object ~1m away. | Introduced `ballKfUpdated`/`goalKfUpdated` flags (set to `true` only when the theta check passes AND `*KF.update()` is actually called). Changed ball decay condition from `!haveBallMeas` to `!ballKfUpdated`. Added goal velocity decay: `if (goalKF.inited && !goalKfUpdated) { goalKF.kx.x1 *= 0.9; goalKF.ky.x1 *= 0.9 }` (gentler 0.9× vs ball's 0.8× since goal is genuinely static). Both decays now fire on every cycle where the KF was not actually updated — covering the theta-rejected-but-ball_valid case that was previously invisible to the old condition. |
...
- **Bug #11 fix untested on hardware.** Kalman velocity decay condition
  changed from `!haveBallMeas` to `!ballKfUpdated`; goal KF decay added.
  Key things to verify in the next log: `ball_now` should no longer drift
  to 1500+mm during theta-rejected stretches; `goal_now` should stay in a
  reasonable range (~800–1500mm for a 1m field); `KICK_POINT_MISSED` should
  not fire from drifting KF estimates; `AT_KICK_POSE` should eventually fire
  as the robot approaches the ball stably.
